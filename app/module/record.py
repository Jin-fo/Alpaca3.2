...
class Record:
    ...
    async def write(self, data: Dict[str, pd.DataFrame]) -> None:
        ...
        for symbol, df in data.items():
            if df is not None and not df.empty:
                task = asyncio.to_thread(self._write_file, symbol, df)
                write_tasks.append(task)
        
        if write_tasks:
            await asyncio.gather(*write_tasks, return_exceptions=True)
    
    def _write_file(self, symbol: str, df: pd.DataFrame) -> None:
    ...
    async def read(self, symbols: Union[str, List[str]]) -> Dict[str, pd.DataFrame]:
        ...
        for symbol in symbols:
            task = asyncio.to_thread(self._read_file, symbol)
    ...
    def _read_file(self, symbol: str) -> Optional[pd.DataFrame]:
